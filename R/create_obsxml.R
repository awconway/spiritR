...
#' requirements for automatic upload of an observational study to the registry
#' 
...
#' collection of data
#' @param obs_design Either: Cohort, Case-Control, Case-Only, Case-Crossover,
#' Ecologic or Community, Family-Based or Other.
#' @param biospec_retention Either: None Retained; Samples With DNA; Samples 
#' Without DNA
#' @param biospec_desc Specify all types of biospecimens to be retained (e.g., 
#' whole blood, serum, white cells, urine, tissue). Limit: 1000 characters.
#' @param timing Either: Retrospective, Prospective; Cross-sectional; Other
#' @param sample_size Planned sample size
#' @param number_groups Number of groups. Enter "1" for a single-group study. 
#' Many observational studies have one group/cohort; case control studies 
#' typically have two.
#' @param study_population A description of the population from which the groups 
#' or cohorts will be selected (for example, primary care clinic, community 
#' sample, residents of a certain town).  Limit 1000 characters
#' @param sampling_method Either: Probability Sample or Non-Probability Sample
...
#' @examples
#' \dontrun{
#' create_obsxml(org_name = "UHNToronto",
...
#'        
#' @export
#' @rdname create_obsxml

create_obsxml <- function(org_name, org_study_id, brief_title, study_acronym,
                         official_title, agency, resp_party_type, 
                         investigator_username, investigator_title, 
                         brief_summary, start_date, 
                         study_compl, primary_compl, obs_design,
                         biospec_retention, biospec_desc, timing, number_groups, 
                         sample_size, study_population, sampling_method,
  ...
  xml2::xml_new_root( "study_collection",
    ...
    xml2::xml_add_child("study_type", "Observational") %>% 
    xml2::xml_add_sibling("observational_design") %>% 
    xml2::xml_add_child("observational_study_design", obs_design) %>% 
    xml2::xml_add_sibling("biospecimen_retention", biospec_retention) %>% 
    xml2::xml_add_sibling("biospecimen_description") %>%
    xml2::xml_add_child("textblock", biospec_desc) %>% 
    
    xml2::xml_find_first("..") %>% 
    xml2::xml_add_sibling("timing", timing) %>% 
    xml2::xml_add_sibling("number_of_groups", number_groups) %>%
    xml2::xml_add_sibling("patient_registry", "false") %>%
    xml2::xml_add_sibling("target_duration_quantity") %>%
    xml2::xml_add_sibling("target_duration_units") %>%
    ...
    xml2::xml_add_child("textblock", study_population) %>% 
    
    xml2::xml_find_first("..") %>% 
    xml2::xml_add_sibling("sampling_method", sampling_method) %>% 
